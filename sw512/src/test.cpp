...
void check(const char* name, double a, double b) {
    constexpr double tol = 1e-12;
    if (std::abs(a - b) > tol) {
        std::cerr << "Test failed for " << name << ": got " << a << ", expected " << b << std::endl;
        throw std::runtime_error("Validation error");
    }
}

int main() {
    auto bodies = solarsystem_ics;
    move_to_center_of_mass(bodies);
    inertial_to_democraticheliocentric_posvel(bodies);

    // Expected values from util.cpp debug
    check("pos[1].x", bodies[1].pos[0], -0.050606442387163809);
    check("pos[1].y", bodies[1].pos[1], -0.46199153044180374);
    check("pos[1].z", bodies[1].pos[2], -0.033112314887493777);
    check("vel[1].x", bodies[1].vel[0], 1.2978664296086504);
    check("vel[1].y", bodies[1].vel[1], -0.095245414385742383);
    check("vel[1].z", bodies[1].vel[2], -0.1267757435000321);

    check("pos[8].x", bodies[8].pos[0], 29.796803635571621);
    check("pos[8].y", bodies[8].pos[1], -2.5135110786022508);
    check("pos[8].z", bodies[8].pos[2], -0.63492520913961792);
    check("vel[8].x", bodies[8].vel[0], 0.014142948749760034);
    check("vel[8].y", bodies[8].vel[1], 0.18292110904074921);
    check("vel[8].z", bodies[8].vel[2], -0.0040928456197298641);
